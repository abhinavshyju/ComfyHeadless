...
def get_path(folder_name, filename):
...
class LoadLatent:
    ...
    def load(self, latent):
        latent_path = get_annotated_path(latent)
    ...
    def IS_CHANGED(s, latent):
        image_path = get_annotated_path(latent)
        m = hashlib.sha256()
...
class CheckpointLoader:
    ...
    def load_checkpoint(self, config_name, ckpt_name):
        ...
        ckpt_path = get_path("checkpoints", ckpt_name)
...
class VAELoader:
    ...
    def load_taesd(name):
        ...
        enc = comfy.utils.load_torch_file(get_path("vae_approx", encoder))
        for k in enc:
            sd["taesd_encoder.{}".format(k)] = enc[k]

        dec = comfy.utils.load_torch_file(get_path("vae_approx", decoder))
        for k in dec:
    ...
    def load_vae(self, vae_name):
        ...
        if vae_name == "pixel_space":
            ...
            if os.path.splitext(vae_name)[0] in self.video_taes:
                vae_path = get_path("vae_approx", vae_name)
            else:
                vae_path = get_path("vae", vae_name)
...
class ControlNetLoader:
    ...
    def load_controlnet(self, control_net_name):
        controlnet_path = get_path("controlnet", control_net_name)
...
class DualCLIPLoader:
    ...
    def load_clip(self, clip_name1, clip_name2, type, device="default"):
        ...
        clip_path1 = get_path("text_encoders", clip_name1)
        clip_path2 = get_path("text_encoders", clip_name2)
...
class CLIPVisionLoader:
    ...
    def load_clip(self, clip_name):
        clip_path = get_path("clip_vision", clip_name)
...
class StyleModelLoader:
    ...
    def load_style_model(self, style_model_name):
        style_model_path = get_path("style_models", style_model_name)
...
class GLIGENLoader:
    ...
    def load_gligen(self, gligen_name):
        gligen_path = get_path("gligen", gligen_name)
...
class LoadImage:
    ...
    def load_image(self, image):
        image_path = get_annotated_path(image)
